...
static void draw_gears(void){
	...
	glVertex3f( -13.3f, 10.0f, z_depth );//top left
	
	glTexCoord2f( 0.0f, 1.0f ); 
	glVertex3f( -13.3f, -10.0f,z_depth );//bottom left
	
	glTexCoord2f( 1.0f, 1.0f );
	glVertex3f( 13.3f, -10.0f, z_depth );	//bottom right
	
	glTexCoord2f( 1.0f, 0.0f );
	glVertex3f( 13.3f, 10.0f, z_depth);	//top right
	glEnd( );
...
int main(int argc, char *argv[]){
	...
	if ((fd = open ("GSTVIDEO_IN_PIPE",  ( O_RDONLY | O_NONBLOCK ) ))<0){
	...
	*/
	...
	load_pipeline(0 ,(char *)"videotestsrc ! video/x-raw,width=640,height=480,framerate=(fraction)30/1 ! queue ! glupload ! video/x-raw(memory:GLMemory),width=640,height=480,format=RGBA ! glfilterapp name=grabtexture ! fakesink sync=false");
